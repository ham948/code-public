...
void lcd_print10x16(unsigned char c){
    unsigned char i, j;
    c -= 48;

    for(i = 0;i<10;i++)
        for(j = 0; j<2; j++){
            lcd_write_data(invert^pgm_read_byte(&clock_num10x16[1+c*21+i*2+j]));
...
void lcd_print25x32(unsigned char c){
    unsigned char i, j;
    c -= 48;
    for(i = 0;i<25;i++)
        for(j = 0;j<4;j++){
            lcd_write_data(invert^pgm_read_byte(&clock_num25x32[1+c*101+i*4+j]));

            if(j == 3) lcd_movCursor(1,-3);
            else lcd_movCursor(0,+1);
        }
