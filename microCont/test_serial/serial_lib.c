...
void usart_recieveStr(char *s){
  int i;
  UCSR0B ^= (1<<RXCIE0);
  while((s[0] == '\n') || (s[0] = usart_receive()) == '\r'); //skips new line, carriage return and assigns first value
  for(i = 0;(s[i]=usart_receive()) != '\r';i++);    //use \0 if between devices, but Putty uses \r when you hit enter
  s[i] = '\0';
  UCSR0B ^= (1<<RXCIE0);
}
/*
